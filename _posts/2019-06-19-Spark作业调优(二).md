...
### 前言
继`Spark作业调优(一)`讲解了`开发调优`与`资源调优`之后，本文继续分析`数据倾斜调优`与`shuffle调优`，以解决更加棘手的性能问题。
...
#### 如何定位导致数据倾斜的代码
数据倾斜只会发生在shuffle过程中。这里给大家罗列一些常用的并且可能会触发shuffle操作的算子：**distinct、groupByKey、reduceByKey、aggregateByKey、join、cogroup、repartition**等。出现数据倾斜时，可能就是你的代码中使用了这些算子中的某一个所导致的。
...
学习转载自[https://tech.meituan.com/spark-tuning-pro.html]( https://tech.meituan.com/spark-tuning-pro.html) 